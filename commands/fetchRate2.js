...
export default async (event) => {
  try {
    const { data } = await axios.get('https://api.themoviedb.org/3/movie/popular?api_key=d870602b8d33fc12e2d09b56f83b190f&language=zh-TW&page=1&region=TW')
    ...
    for (let i = 0; i < 10; i++) {
      ...
      bubble.hero.url = 'https://www.themoviedb.org/t/p/w300' + data.results[i].poster_path
      ...
      bubble.footer.contents[0].action.uri = `https://www.themoviedb.org/movie/${data.results[i].id}`
      movie.push(bubble)
    }
